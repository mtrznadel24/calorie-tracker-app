...
def get_fridge_products(
    ...
    if is_favourite:
        query = query.filter(FridgeProduct.is_favourite)
...
def update_fridge_product(
    db: Session, fridge_id: int, product_id: int, data: FridgeProductUpdate
):
    return update_fridge_object(
...
def get_fridge_meals(
    ...
    if is_favourite:
        query = query.filter(FridgeMeal.is_favourite)
    return query.offset(skip).limit(limit).all()
...
def update_fridge_meal(
    db: Session, fridge_id: int, meal_id: int, data: FridgeMealUpdate
):
    return update_fridge_object(
...
def get_fridge_meal_nutrient_sum(
    db: Session, fridge_id: int, meal_id: int, nutrient_type: NutrientType
):
...
def get_fridge_meal_macro(db: Session, fridge_id: int, meal_id: int):
    ...
    return {
        field.value.replace("_100g", ""): get_fridge_meal_nutrient_sum(
            db, fridge_id, meal_id, field
        )
        for field in fields
    }
...
def add_fridge_meal_ingredient(
    db: Session, fridge_id: int, meal_id: int, data: FridgeMealIngredientCreate
):
...
def get_fridge_meal_ingredients(db: Session, fridge_id: int, meal_id: int):
    return (
        ...
        .join(FridgeMeal)
        .filter(FridgeMeal.id == meal_id, FridgeMeal.fridge_id == fridge_id)
        .all()
    )


def get_fridge_meal_ingredient(
    db: Session, fridge_id: int, meal_id: int, ingredient_id: int
):
    return (
        db.query(FridgeMealIngredient)
        .join(FridgeMeal)
...
def update_fridge_meal_ingredient(
    db: Session,
    fridge_id: int,
    meal_id: int,
    ingredient_id: int,
...
def delete_fridge_meal_ingredient(
    db: Session, fridge_id: int, meal_id: int, ingredient_id: int
):
    get_fridge_object_or_404(db, FridgeMeal, fridge_id, meal_id)
