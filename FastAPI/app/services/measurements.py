...
async def create_measurements(db: AsyncSession, user_id: int, data: MeasurementsCreate) -> Measurement:
    weight_in = None
    if data.weight is not None:
        weight_in = await create_weight(db, user_id, data.weight) # NOTE Weight is commited immediately, will be fixed with repositories
...
async def get_measurements(db: AsyncSession, user_id: int, measurement_id: int) -> Measurement:
...
async def delete_measurements(db: AsyncSession, user_id: int, measurements_id: int) -> Measurement:
...
async def create_weight(db: AsyncSession, user_id: int, data: WeightCreate) -> Weight:
    result = await db.execute(select(Weight).where(Weight.user_id == user_id, Weight.date == data.date))
...
async def get_user_weight(db: AsyncSession, user_id: int, weight_id: int) -> Weight | None:
