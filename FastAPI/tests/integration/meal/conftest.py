...
def ingredient_factory(session):
    async def factory(meal, weight, product_name, calories_100g, proteins_100g, fats_100g, carbs_100g):
        ingredient = MealIngredient(weight=weight, meal_id=meal.id)
        ingredient.details = MealIngredientDetails(product_name=product_name,
...
async def sample_meal_with_ingredient(session, user):
    ...
    ingredient.details = MealIngredientDetails(product_name="Banana", calories_100g=89, proteins_100g=1.1, fats_100g=0.3, carbs_100g=23)
    session.add(ingredient)
    await session.commit()
