...
from app.fridge.models import Fridge, FoodCategory, FridgeProduct, FridgeMeal, FridgeMealIngredient
from app.meal.models import MealIngredient
...
def fridge_product_factory(session, fridge):
    async def factory(product_name, calories_100g, proteins_100g, fats_100g, carbs_100g, category: FoodCategory, is_favourite: bool):
...
def fridge_meal_ingredient_factory(session, fridge):
    async def factory(meal, weight, product):
        ingredient = FridgeMealIngredient(fridge_meal_id=meal.id, weight=weight, fridge_product_id=product.id)
...
async def sample_fridge_meal_with_ingredient(session, fridge, sample_fridge_product):
    ...
    session.add(meal)
    await session.flush()
    ingredient = FridgeMealIngredient(weight=50, fridge_meal_id=meal.id, fridge_product_id=sample_fridge_product.id)
